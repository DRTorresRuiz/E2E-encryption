...
#####
connected              = asyncio.Semaphore(0)   # Control the reg. process
msg_1                  = False                  # Flag for received Reg. Request
msg_3                  = False                  # Flag for received message 3
msg_5                  = False                  # Flag for received message 5
msg_7                  = False                  # Flag for received message 7
verificationCode       = ""                     # Verification code to auth device
connection_failed      = False                  # Check if reg. process fails
newDevice              = {}                     # Device info to save after reg.
shared_key             = ""                     # First shared key after dh or ecdh
symmetricAlgorithm     = ""                     # Symmetric alg. used in reg. process
asymmetricAlgorithm    = ""                     # Hand-shake algorithm used
encriptor              = None                   # Cipher used during reg. process
...
def print_error_registration( error_message ):
    """
        Prints the error message and set the global
        variable connection_failed with True and so
...
def on_receive_message_5( client, userdata, msg ):
    """
        After receiving confirmation from device,
        it will display or introduce a verification code.
    """
    global encriptor
    ...
    if encriptor != None:
        ...
        if msg.get( "status", "ERROR" ) == "OK":
            ...
            if deviceType != "" and deviceType == "I":
                ...
                # next message.
                confirmation["new_key"] = utils.generate_new_key( symmetricAlgorithm )
                message = add_header_message( confirmation, userdata,\
                    REGISTRATION_TOPIC, 6 )
                utils.send( client, encriptor, message )
                # After sending the message, modify encriptor with the new
                #  key before generated.
                encriptor = utils.modify_encriptor( confirmation["new_key"],\
                    symmetricAlgorithm ) 
            return True
    return False
...
def getRegisteredDevices():
    """
        Returns a dictionary with all devices 
        listed in the REGISTERED_DEVICE_FILE.
    """
    devices = {}
...
@click.option( '-p', '--password', 'password', required=True, type=str,\
    prompt=True, hide_input=True, help="The password for the user to connect to \
        the MQTT Serve. If you do not include this option, a prompt will appear to \
        you introduce the password." )
def register( server, port, user, password ):
    ...
    if not connected.locked():
        ...
        message = requests.post( KMS_SERVER_URL+"register-device",\
            json = post_message, auth=( user, password ) )
...
def list_devices():
    """
        Print the list of the Registered Devices.
    """
    devices = getRegisteredDevices()
    if devices == {}:
...
def on_message( client, userdata, msg ):
    """
        Receiving messages from all topics subscribed.
    """
    ...
    if msg.topic in topics_subscribed:
        ...
        if match:
            ...
            # Ask for the secrets of the device to KMS
            secrets = requests.post( KMS_SERVER_URL+"get-key",\
                json = secret_request,\
                auth=( userdata["user"], userdata["password"] ) ).json()
            message = get_data_message( str( msg.payload.decode( "utf-8" ) ),\
                secrets.get( "secrets", None ), secrets.get( "symmetric", None ) )
            if message != "":
                # If the message could be decrypted... printed.
                print( message )
...
@click.option( '-p', '--password', 'password', required=True, type=str,\
    prompt=True, hide_input=True, help="The password for the user to connect to \
        the MQTT Serve. If you do not include this option, a prompt will appear \
        to you introduce the password." )
def connect( server, port, user, password ):
    """
      Listen and subscribe to all data topic in REGISTERED_DEVICE_FILE.  
    """
    global topics_subscribed
...
@click.option( '-u', '--user', 'user', required=True, type=str,\
    help="The user to connect to the MQTT Serve." )
@click.option( '-p', '--password', 'password', required=True, type=str,\
    prompt=True, hide_input=True, help="The password for the user to connect \
        to the MQTT Serve. If you do not include this option, a prompt will \
        appear to you introduce the password." )
@click.option( '-t', '--topic', 'topic', required=True, default="", type=str,\
    help="The topic you want to subscribe." )
def listen_topic( server, port, user, password, topic ):
    """
        Listen and subscribe to an specified topic.
    """
    client = connect_MQTT( server, port, user, password, on_message )
    if topic != "":
...
@click.option( '-u', '--user', 'user', required=True, type=str,\
    help="The user to connect to the MQTT Serve." )
@click.option( '-p', '--password', 'password', required=True, type=str,\
    prompt=True, hide_input=True, help="The password for the user to \
        connect to the MQTT Serve. If you do not include this option, a \
        prompt will appear to you introduce the password." )
@click.option( '-i', '--idDevice', 'idDevice', required=True, default="",\
    type=str, help="Introduce the id of the device you want to remove." )
def remove_device( user, password, idDevice ):
    """
        Remove the specified device from platform and KMS.
    """
    post_message = {
        # Data information to sent to KMS
        "id": idDevice
    ...
    with open( REGISTERED_DEVICE_FILE, 'w' ) as file:
        ...
        json.dump( devices, file, indent=4 )
    message = requests.post( KMS_SERVER_URL+"remove-device",\
        json = post_message, auth=( user, password ) )
